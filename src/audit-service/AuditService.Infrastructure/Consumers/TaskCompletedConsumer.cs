...
public sealed class TaskCompletedConsumer(AuditDbContext dbContext) : IConsumer<TaskCompletedIntegrationEventV1>
{
    public async Task Consume(ConsumeContext<TaskCompletedIntegrationEventV1> context)
    {
        ...
        using (LogContext.PushProperty(LogPropertyKeys.UserId, context.Message.CompletedByUserId))
        {
            var record = new AuditRecord
            {
                ...
                EventType = nameof(TaskCompletedIntegrationEventV1),
                EntityId = context.Message.EventId,
                UserId = context.Message.CompletedByUserId,
