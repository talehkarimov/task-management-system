...
public sealed class TaskCreatedConsumer(AuditDbContext dbContext) : IConsumer<TaskCreatedIntegrationEventV1>
{
    public async Task Consume(ConsumeContext<TaskCreatedIntegrationEventV1> context)
    {
        ...
        using (LogContext.PushProperty(LogPropertyKeys.UserId, context.Message.ReporterUserId))
        {
            var record = new AuditRecord
            {
                ...
                EventType = nameof(TaskCreatedIntegrationEventV1),
                EntityId = context.Message.EventId,
                UserId = context.Message.ReporterUserId,
